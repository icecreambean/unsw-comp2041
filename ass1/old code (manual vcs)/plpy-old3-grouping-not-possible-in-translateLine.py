...
sub translateLine {
    ...
    # and in perl, they're probably compatible with almost everything
    #   (see below)
    # ================ match to a data type ================
    # note: matching to expression code mostly identical each time
    # (would modularise exp match but $1 doesn't transfer between functions)
    # TODO: match to "my", "our"
    if ($pl[0] =~ /^([!~]|not)/) {
        # "!" and "~" operator could occur anywhere....
    ...
    if ($pl[0] =~ /^('.*?[^\\]')/ || $pl[0] =~ /^(".*?[^\\]")/ ||
        ...
        return determineExpressionTranslation($result);
    }
    ...
    if ($pl[0] =~ /^(\$\w+)/ || $pl[0] =~ /^(\${.*?})/) {
        ...
        # below doesn't work with grouping
    ...
    # ================ match to grouping ================
    # should grouping have such low priority? it doesn't really matter?
    # FIXME: this doesnt work at all!!
    # grouping also incompatible with expressions
    if ($pl[0] =~ /^\(/) {
        my $result = "(";
        $pl[0] = substr $pl[0], 1;
        while ($pl[0] !~ /^\)/) { # assume valid perl
            $result .= translateLine();
        }
        $result .= ")";
        $pl[0] = substr $pl[0], 1;
        return $result . translateLine();
    }
...
sub translateExpression {
    ...
    # no whitespace to be added for "!" or "~": only takes one argument
    $arg .= translateNextWhitespace();
    $arg .= " " if ($pl[0] !~ /^([!~]|not)/ && $arg !~ /\s$/);
    ...
    my $result = translateOperator($arg) . translateLine() . translateNextWhitespace();
    return $result;
}
sub isOperatorNext {
    # matches for:
...
sub getOperator {
    # grabs an operator and updates $pl[0] WITHOUT translating
    # (note): func doesn't see much usage because i wrote it late
    if ($pl[0] =~ /^(and|or|not)(\s*)/ ||
        $pl[0] =~ /^(<=>)(\s*)/ ||
        $pl[0] =~ /^([|][|]|&&|<=|>=|!=|==|\*\*|>>|<<)(\s*)/ ||
        $pl[0] =~ /^([+\-\/%*!<>|^&~=])(\s*)/)
    {
        my $result = $1;
        $result .= $2 if defined $2;
        $pl[0] = substr $pl[0], $+[0];
        return $result;
    }
    return ""; # or better to die?
}
sub isGroupedAfterOperator {
    # hacky check to see if there is a bracket after an operator
    # WARNING: no defensive check because lazy
    my $initial_contents = "";
    while (isOperatorNext()) {
        $initial_contents .= getOperator();
    }
    my $isGrouped = 0; # false
    $isGrouped = 1 if ($pl[0] =~ /^\s*\(/);
    # restore $pl[0]
    $pl[0] = $initial_contents . $pl[0];
    return $isGrouped;
}
sub determineExpressionTranslation {
    my ($result) = @_;
    if (isOperatorNext()) {
        if (isGroupedAfterOperator()) {
...
sub translateControlFlow {
    ...
    if ($command =~ /^(if|elsif|while)/) {
        ...
        # grab one argument (should be a variable)
        $py_line .= translateLine();
        # grab round bracket
        $pl[0] =~ /^(\s*\(\s*)/;
        $pl[0] = substr $pl[0], $+[0];
    ...
    if ($pl[0] =~ /^\s*elsif|else/) {
        return $py_line; # throw away the whitespaces
    }
